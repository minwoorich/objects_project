...
public class FakeOrderCommendRepository extends BaseFakeOrderRepository implements OrderCommendRepository {
    ...
    public Order save(Order order) {
        if (order.getId() == null || order.getId() == 0) {
            ReflectionTestUtils.setField(order, "id", ++super.autoGeneratedId, Long.class);
        } else {
            getInstance().data.removeIf(item -> Objects.equals(item.getId(), order.getId()));
        }
        getInstance().data.add(order);
        return order;
    }
