...
@Test
@DisplayName("SID를 이용한 정기결제 API")
@WithMockCustomUser
public void kakaoPayApprovedSubscribtion() throws Exception{
    Amount amount = Amount.builder().totalAmount(100L)
            .taxFreeAmount(0L)
            .discountAmount(0L).build();
    ...
    LocalDateTime approvedAt = LocalDateTime.of(2024, 2, 06, 9, 24);

    KakaoPayApproveResponse response = KakaoPayApproveResponse.builder().aid("aid")
            .tid("tid")
            .cid("cid")
            .sid("sid")
            .partnerOrderId("partnerOrderId")
    ...
    given(memberShipService.kakaoPaySubsApprove()).willReturn(response);
    //when,then

    mockMvc.perform(post("/membership/subscription")
                    .header(HttpHeaders.AUTHORIZATION, "bearer AccessToken")
                    .contentType(MediaType.APPLICATION_JSON))
            .andExpect(status().isOk())
            .andDo(print())
            .andDo(document("kakaoPaySubs-subsApproved",
                    preprocessRequest(prettyPrint()),
                    preprocessResponse(prettyPrint()),
    responseFields(
            fieldWithPath("code").type(JsonFieldType.NUMBER)
